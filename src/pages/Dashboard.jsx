...
function upsertMetric(map, key, item) {
  ...
  if ((item.score || 0) >= PASSING_SCORE || item.passed) {
    current.passed += 1
  }
...
function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    history.forEach((session) => {
      session.items.forEach((item) => {
        ...
        if (item.type === QUESTION_TYPES.coding && score < PASSING_SCORE) {
          failedCodingQuestionIds.add(item.id)
        }
      })
    })

    const topicPerformance = finalizeMetrics(topicMap).slice(0, 6)
    const difficultyPerformance = finalizeMetrics(difficultyMap)

    const weakAreas = finalizeMetrics(weakAreaMap)
      .filter((entry) => entry.averageScore < 6.5 || entry.passRate < 65)
      .sort((left, right) => {
        const leftSeverity = (6.5 - left.averageScore) + (65 - left.passRate) / 20
        const rightSeverity = (6.5 - right.averageScore) + (65 - right.passRate) / 20
    ...
    return {
      ...
      weakAreas,
      streaks: {
        daily: getCurrentStreak(uniqueDays, 1),
        weekly: getWeeklyStreak(uniqueWeeks),
      },
  ...
  function handlePracticeWeakArea() {
    ...
    startInterview(target.primaryType, {
      categories: [target.label],
      label: `Weak area: ${formatLabel(target.label)}`,
    })
    navigate('/interview')
  }
        ...
        <>
          ...
          <div className="card-grid dashboard-grid">
            ...
            <div className="card stat-card">
              ...
              <span className="stat-value">{stats.streaks.daily} day{stats.streaks.daily === 1 ? '' : 's'}</span>
            </div>
          </div>
          ...
          <div className="dashboard-panels">
            <div className="card analytics-card">
              <div className="history-header">
                <h2>Quick actions</h2>
                <span className="badge badge-soft">Stay focused</span>
              </div>
              <div className="quick-actions">
                <button className="primary-button" type="button" onClick={handlePracticeWeakArea}>
                  Practice weak area
                </button>
                <button
                  className="secondary-button"
                  type="button"
                  onClick={handleRetryFailedCoding}
                  disabled={!stats.failedCodingQuestionIds.length}
                >
                  Retry failed coding tasks
                </button>
              </div>
              <div className="quick-actions-meta">
                <span>
                  Weekly streak: <strong>{stats.streaks.weekly}</strong> active week{stats.streaks.weekly === 1 ? '' : 's'}
                </span>
                <span>
                  Latest focus: <strong>{stats.latestType ? stats.latestType.toUpperCase() : '–'}</strong>
                </span>
              </div>
            </div>
            ...
            <div className="card analytics-card">
              <div className="history-header">
                ...
                <span className="badge badge-soft">Top patterns</span>
              </div>
              <div className="metric-list">
                {stats.topicPerformance.map((entry) => (
                  <div key={entry.label} className="metric-row">
                    <div>
                      <div className="metric-title">{formatLabel(entry.label)}</div>
                      <div className="metric-subtitle">{entry.attempts} attempts • {entry.passRate.toFixed(0)}% pass rate</div>
                    </div>
            ...
            <div className="card analytics-card">
              <div className="history-header">
                ...
                <span className="badge badge-soft">Balanced practice</span>
              </div>
              <div className="metric-list">
                {stats.difficultyPerformance.map((entry) => (
                  <div key={entry.label} className="metric-row">
                    <div>
                      <div className="metric-title">{formatLabel(entry.label)}</div>
                      <div className="metric-subtitle">{entry.attempts} prompts • {entry.passRate.toFixed(0)}% pass rate</div>
                    </div>
                    <div className="metric-value">{entry.averageScore.toFixed(1)}/10</div>
                  </div>
                ))}
          ...
          <div className="dashboard-panels dashboard-panels-secondary">
            <div className="card analytics-card">
              <div className="history-header">
                <h2>Weak areas</h2>
                <span className="badge badge-soft">Needs attention</span>
              </div>
              <div className="metric-list">
                {stats.weakAreas.length ? (
                  stats.weakAreas.map((entry) => (
                    <div key={entry.label} className="metric-row metric-row-warning">
                      <div>
                        <div className="metric-title">{formatLabel(entry.label)}</div>
                        <div className="metric-subtitle">
                          Avg {entry.averageScore.toFixed(1)} • Pass rate {entry.passRate.toFixed(0)}%
                        </div>
                      </div>
            ...
            <div className="card history-card">
              ...
              <div className="history-summary-row">
                {Object.entries(stats.countsByType).map(([type, count]) => (
                  <span key={type} className="badge badge-soft">
                    ...
                    <div className="history-main history-main-stacked">
                      ...
                      <div>
                        ...
                        <div className="history-topics">
                          {(item.analytics?.topicTags || []).slice(0, 3).map((tag) => (
                            <span key={`${item.id}-${tag}`} className="history-topic-pill">
                              {formatLabel(tag)}
                            </span>
                          ))}
                        </div>
